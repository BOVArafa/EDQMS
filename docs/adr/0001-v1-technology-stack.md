...
**Frontend.** Vue 3 with the Composition API and `<script setup>`, in **TypeScript**, bundled with Vite. UI components via **shadcn-vue** (Reka UI) on **Tailwind CSS**, applying the Siemens Energy Design System tokens in the Tailwind theme, dark mode by default. Charts via shadcn-vue *charts* (Unovis); ECharts (`vue-echarts`) remains an exception for charts the default library does not cover well. The frontend is a **REST API client**, with no duplicated business logic.
